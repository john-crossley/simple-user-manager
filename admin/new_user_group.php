...
ensure_login();
$user = get_user();
check_user_access($user, 'createUserGroups', array(
    'redirect' => 'admin/'
));
...
<div class="col-sm-3 col-md-2 sidebar">
    <?php echo get_admin_sidebar('create-group') ?>
</div>
    ...
    <form method="post" action="<?= root_path('admin/new_user_group.php') ?>">
        <div class="form-group has-<?= form_has_error('roleName') ?>">
            ...
            <input type="text" class="form-control" id="role_name" name="roleName"
                   placeholder="Enter the name of the new role" value="<?= form_has_value('roleName') ?>">
            <small
                class="help-block"><?= form_has_message('roleName', 'Only alpha-numeric characters allowed') ?></small>
        </div>
        <!--//.form-group-->
