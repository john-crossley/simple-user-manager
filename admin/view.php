...
<div class="active tab-pane" id="edit-account">
    ...
    <form method="post" action="<?= root_path('admin/view.php?user=' . $user->id) ?>" enctype="multipart/form-data">
        ...
        <div class="col-lg-6">
            ...
            <div class="form-group has-<?= form_has_error('redirect_to') ?>">
                ...
                <input type="text" class="form-control" id="redirect_to" name="redirect_to"
                       placeholder="Enter where the user should be redirect" value="<?= $user->redirect_to ?>">
                <small
                    class="help-block"><?= form_has_message('redirect_to', root_path() . '<span class="url"></span>') ?></small>
            </div>
            <!--//.form-group-->
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="member-status" name="roleId">
                    ...
                    <?php foreach (Role::getSystemUserGroups() as $group): ?>
                        <option
                            value="<?= $group->role_id ?>" <?= status($group->role_id, $role->role_id) ?>><?= $group->role_name ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <!--//.form-group-->
            ...
            <div class="form-group">
                <div class="checkbox">
                    <label>
                        <input type="checkbox"
                               name="banned_from_sending_personal_messages" <?= is_checked($user->banned_from_sending_personal_messages) ?>>
                    ...
                    <?php foreach ($pages as $page): ?>
                        <li class="list-group-item">
                            ...
                            <div class="pull-right">
                                ...
                                <a href="#protection_modal" data-toggle="modal" data-id="<?= $page->id ?>"
                                   class="label label-success">How to protect</a>
                            </div>
                        </li>
                    <?php endforeach; ?>
                <?php endif; ?>
            ...
            <form method="POST" action="<?= root_path('admin/view.php?user=' . $user->id) ?>">
                <div class="form-group has-<?= form_has_error('access_area_path') ?>">
                    <div class="input-group">
                        <input type="hidden" name="user_id" value="<?= $user->id ?>">
                        <input type="hidden" name="csrf" value="<?= get_csrf_token() ?>">
                        <input type="text" class="form-control url" name="access_area_path" placeholder="Eg: /"
                               value="/">
...
<div class="tab-pane" id="delete-account">
    ...
    <form method="POST" action="<?= root_path('admin/view.php?user=' . $user->id) ?>">
        ...
        <input type="hidden" name="csrf" value="<?= get_csrf_token() ?>">
        <input type="hidden" name="user_id" value="<?= $user->id ?>">
        <button class="btn btn-danger" onclick="return confirm('You sure you want to delete this account?')">I
            understand, delete this account!
        </button>
    </form>
</div>
<!--//#access-areas-->
