                ...
                <form method="POST" action="<?= root_path('messages.php') ?>">
                    <table class="table table-hover table-condensed">
                        ...
                        <?php foreach (get_received_messages($user->id) as $message): ?>
                            <tr <?= ($message->read == 0) ? "class=''" : '' ?>>
                                ...
                                <td><span
                                        class="label label-<?= ($message->read == 0) ? 'danger' : 'success' ?>"><?= ($message->read == 0) ? 'Unread' : 'Read' ?></span>
                                </td>
                                ...
                                <td>
                                    <a href="#view-message" data-task="view_message_inbox"
                                       data-message="<?= $message->id ?>" class="btn btn-success btn-xs">View</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    ...
                    <div class="form-group">
                        <button class="btn btn-danger btn-xs pull-right"
                                onclick="return confirm('Sure you want to delete these messages?');">Delete Selected
                        </button>
                    </div>
                    <!--//.form-group-->
                </form>
