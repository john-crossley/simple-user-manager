            ...
            <?php if (pm_system_enabled()): ?>
                <?php if (_logged_in()): ?>
                    ...
                    <?php if (($tmp_user->id == $user->id) || ($tmp_user->banned_from_sending_personal_messages > 0)): ?>
                        <button type="button" class="btn btn-success" disabled="disabled">Unable to send a message
                        </button>
                    <?php else: ?>
                        <?php if ($tmp_user->receive_personal_messages > 0): ?>
                            <a href="#send-personal-message" data-toggle="modal" class="btn btn-success">Send
                                Message</a>
                        <?php endif; ?>
                    <?php endif; ?>
                <?php else: ?>
                ...
                <div class="modal-body">
                    <div class="form-group" id="subject-block">
                        ...
                        <input type="text" class="form-control" id="title" name="title"
                               placeholder="I have found the meaning of life!">
                    </div>
                    <!--//.form-group-->

                    <div class="form-group">
                        <label for="message" class="control-label">Message</label>
                        <textarea class="form-control high-textarea" id="message" name="message" cols="1" rows="1"
                                  placeholder="On second thoughts... No I have not"></textarea>
